...
class MyKanni:
  ...
  def start(cls, path, filter_shape, sigma,low_pr,high_pr):
    ...
    PrintImg.print_n_m_imgs(2, 2, ["prepared image.\nfilter_shape: " + str(filter_shape[1]) + " " + str(filter_shape[0]) + ". sigma: " + str(sigma), "gradient_magnitude", "suppressed", "result_of_method\nlow_pr: " + str(low_pr) + ", high_pr: " + str(high_pr)],
                            [img, gradient_magnitude, suppressed,resulf_of_method])
  ...
  def prepare_img(cls, path, filter_shape, sigma):
    ...
    kernal = MyGaussianBlur.gaussian_filter(filter_shape, sigma)
    return MyGaussianBlur.my_filter(img, kernal)
  ...
  def search_for_gradients(cls, img):
    gradient_x = MyGaussianBlur.my_filter(img, cls.get_sobel_operator_x())
    gradient_y = MyGaussianBlur.my_filter(img, cls.get_sobel_operator_y())
